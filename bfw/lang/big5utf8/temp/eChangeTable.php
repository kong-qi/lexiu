...
<form name="ebakchangetb" method="post" action="phome.php" onsubmit="return check();">
  <table width="100%" border="0" align="center" cellpadding="3" cellspacing="1" bgcolor="#0472BC">
    ...
    <tr> 
      <td height="25" bgcolor="#FFFFFF"> <table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#DBEAF5">
          <tr> 
            <td width="22%">&nbsp;</td>
            <td> [<a href="#ebak" onclick="javascript:window.open('ListSetbak.php?mydbname=<?=$mydbname?>&change=1','','width=550,height=380,scrollbars=yes');">導入設置</a>]&nbsp;&nbsp;&nbsp;[<a href="#ebak" onclick="javascript:showsave.style.display='';">保存設置</a>]</td>
          </tr>
          <tr id="showsave" style="display=none">
            <td>&nbsp;</td>
            <td>保存文件名:setsave/ 
              <input name="savename" type="text" id="savename" value="<?=$_GET['savefilename']?>">
              <input name="Submit4" type="submit" id="Submit4" onClick="document.ebakchangetb.phome.value='DoSave';" value="保存設置">
...
<select name="dbchar" id="dbchar">
              ...
              <font color="#666666">(從mysql4.0導入mysql4.1以上版本需要選擇固定編碼，不能選自動)</font></td>
          </tr>
          <tr> 
            <td>存放目錄</td>
            <td height="23"> 
          ...
          <tr>
            ...
            <td height="23">導入方式: 
              ...
              完整插入，<input name="bakstrufour" type="checkbox" id="bakstrufour" value="1"<?=$dbakstrufour==1?' checked':''?>><a title="需要轉換數據表編碼時選擇">轉成MYSQL4.0格式</a>, 每組備份間隔： 
