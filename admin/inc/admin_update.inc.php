...
$sqlshow=$conn->selectall("".DB_EXT."admin","where kq_uuid='".$id."'");
                ...
                <select name="kq_groupid" id="kq_groupid">
                  ...
                  $gpsql=$conn->selectone(DB_EXT."admingroup","id,kq_name","");
		              while($group_r=$conn->result($gpsql)){
          			  if($group_r['gp_id']==$show_r['kq_groupid']){
          				  echo '<option value="'.$group_r['id'].'" selected="selected">'.$group_r['kq_name'].'</option>';
          				}else{
