...
require_once(CLASS_PATH."thumbs.class.php");
$pidid=@$_POST['picid'];
$ylpic=$_POST['ylpic'];
$filepicname=$_POST['filepicname'];
$list='';
$upsize_w='';
$upsize_h='';
$bili='';
...
$php_url =substr(ADMIN_URL,0,strrpos(substr(ADMIN_URL,0,strlen(ADMIN_URL)-1),'/')+1);//设置删除最后一个/
$php_url=substr($php_url,0,strrpos(substr($php_url,0,strlen($php_url)-1),"/")+1);
//文件保存目录路径
$save_path = substr(ADMIN_PATH,0,(strrpos(substr(ADMIN_PATH,0,strlen(ADMIN_PATH)-1),'/')+1))."updatefile/";
//文件保存目录URL
$save_url ='/updatefile/';
//定义允许上传的文件扩展名
$ext_arr = array(
...
function alert($msg) {
	...
	echo $msg;
	echo "点击<a href='../../inc/updatepic.php?pidid=".$pidid."&ylpic=".$ylpic."&file=".$filepicname.$sizeurl."'>重新上传</a>";
	exit();
...
function pic_url($str,$size=""){
	...
	if($str){
		if(strstr($str,"http://") or strstr($str,"HTTP://")){
		    ...
		    $picurl="updatefile/".SUOLUTDIR."/".$size.$str;
		}
	}else{
